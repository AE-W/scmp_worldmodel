...
def h5_to_json(h5_filename):
    ...
    :param h5_filename: input HDF5 filename.
    :param json_filename: output JSON filename.
    """
    with h5py.File(h5_filename, 'r') as h5file:
        result = {}  # holds the converted data
        
        def recursive_convert(group, result):
            """
            Recursively convert HDF5 file contents into a dict.
            
            :param group: HDF5 group or root being processed.
            :param result: dict that stores the conversion result.
            """
            for key, item in group.items():
                if isinstance(item, h5py.Dataset):  # dataset -> read directly
                    result[key] = item[()].tolist()  # convert numpy array to list
...
def process_frames_flexibly(frames, vae, batch_size=40):
    with torch.no_grad():
        n_frames = len(frames)
        # compute how many full batches are needed and how many frames the last batch must contain
        n_full_batches = n_frames // batch_size
        remainder = n_frames % batch_size
        
        # initialize list to hold processed batches
        processed_batches = []
        ...
        # if leftover frames remain, handle them
        if remainder > 0:
            last_batch = frames[-remainder:] # take the last `remainder` frames
...
def main(args,rank):
    ...
    imageio.imwrite('figures/rt1_env/first_frame.png', ori_video[0])  # save as PNG
...
def smooth_ones(arr):
    n = len(arr)
    result = [0.0] * n  # initialize result array
    i = 0
    
    while i < n:
        if arr[i] == 1:
            # find start/end indices of a run of consecutive 1's
            start = i
            ...
            length = end - start
            # handle runs shorter than 10
            if length < 10:
                mid_point = length // 2
                # for runs of <10 consecutive 1's, smoothing must consider overlap
                for j in range(length):
                    # smooth first half; clamp to <=1.0 with min
