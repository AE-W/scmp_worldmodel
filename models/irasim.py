...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., use_lora=False, attention_mode='math', block_idx=-1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.attention_mode == 'xformers': # cause loss nan while using with amp
        ...
        elif self.attention_mode.startswith('sc_int8'):
            from .sc_integration import (
                sc_qk_matmul, sc_av_matmul, sc_linear_forward,
                configure, is_op_enabled,
            )
            ...
            if is_op_enabled("qk", b):
                attn = sc_qk_matmul(q_scaled, k, sc_prec=8)
            else:
                attn = q_scaled @ k.transpose(-2, -1)
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)

            if is_op_enabled("av", b):
                x = sc_av_matmul(attn, v, sc_prec=8).transpose(1, 2).reshape(B, N, C)
            else:
                x = (attn @ v).transpose(1, 2).reshape(B, N, C)
...
class TransformerBlock(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, block_idx=-1, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, block_idx=block_idx, **block_kwargs)
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio)
        approx_gelu = lambda: nn.GELU(approximate="tanh")
        _mode = block_kwargs.get('attention_mode', 'math')
        if isinstance(_mode, str) and _mode.startswith('sc_int8'):
            from .sc_integration import SCMlp
            self.mlp = SCMlp(in_features=hidden_size, hidden_features=mlp_hidden_dim, act_layer=approx_gelu, drop=0, block_idx=block_idx)
        else:
...
class IRASim(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, attention_mode=attention_mode, block_idx=i) for i in range(depth)
        ])
